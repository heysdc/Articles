...
6. 作用域链（scope chain）由变量对象组成，其前端指向当前的执行环境的变量对象，上一级为包含环境的变量对象，直至全局环境的变量对象，保证了对有权访问的所有变量和函数的有序访问
  ...
  funcD(funcB) // 结果为‘a’, 作用域链所包含的变量对象（更正，之前错误的想法为作用域链在定义时被决定，只是大致的活动对象定下来了，而作用域是由活动对象构成的，具体可参照下面的闭包部分），在函数定义的时候就已经被决定了，但在函数执行的时候才生成（参见高级js第三版P178）
...
3. 闭包的定义：js高级编程给的定义，有权访问另一个**函数作用域中**的变量的**函数**
  ...
  es6引入let const有了非函数作用域之后，就不一定是函数作用域了，可参考1中的例子，但一定是**函数**，因为函数内引用的外部变量的值是运行时确定的
...
7. **函数调用时发生了什么？**
  ...
  - 执行环境(execution context): 执行的时候才存在的环境，执行环境定义了变量或者函数有权访问的其它数据，被作用域分成了不同层级的执行环境，如当前函数作用域执行环境，全局执行环境等，一般说某函数的执行环境为该函数的函数作用域执行环境，而不是其能访问到的所有执行环境
  ...
  - 活动对象(activation object)：执行流所执行过的变量和参数，可以被后续访问到

  - 作用域链(scope chain)：由于标识符解析是不会解析到任何执行环境中的**非活动对象**，在一个环境中执行时，会创建活动对象组成的作用域链，本质上是一个指向变量对象的指针列表

    虽然书上（高级编程P73）说创建变量对象的作用域链，但又补充如果为函数执行环境，将活动对象作变量对象，但其实全局活动对象也是一样的，执行流后的定义的非活动对象是访问不到的
